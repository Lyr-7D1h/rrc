...
pub struct Robot {
    ...
    rigid_body_set: MultibodyJointSet,
...
impl Robot {
    fn new() -> Robot {
        ...
        let collider = ColliderBuilder::trimesh(vec![point![1.0, 2.0, 3.0]], vec![[0, 0, 0]])
            ...
            .density(7.85)
            .build();
        let collider = collider_set.insert_with_parent(collider, arm, &mut rigid_body_set);
...
impl SimulationServer {
    pub async fn bind<A: ToSocketAddrs>(addr: A) -> Result<SimulationServer, Box<dyn Error>> {
